...
- Tablets self-enroll on first boot (`register_device`, keyed by hardware ANDROID_ID); any device — claimed or not — can re-enroll itself after a data wipe with whatever name the field engineer types (0008 relaxed this; the name just renames the robot, history is keyed by hardware id so nothing is lost). The dashboard's "serbest bırak" action remains as a last resort. They append one row to `grants` per QR shown (queued locally while offline, deduped by `client_uuid`), and poll `fetch_config` every 60s on the invite screen. A higher `version` resets local limit counters and applies the new promos; lifetime totals (`lifetime_usage_view`) are never reset. Tablets also ping `device_heartbeat` every 5 minutes in every screen, so `last_seen_at` tracks power state.
...
```sql
select * from usage_view order by robot_id, amount;          -- used per robot × coupon (current config period)
select * from lifetime_usage_view order by robot_id, amount; -- lifetime, never resets

...
What it does:
- **Günlük kupon**: stacked daily columns (one segment per coupon amount) with a Filo / Robot toggle and a Kampanya (from 3 Sep 2026) / 7 / 14 / 30-day range, figures for today / range total / top coupon, hover tooltips and a table view. Reads `daily_usage_view` (migration 0009); days are Istanbul calendar days.
